...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate training data from CSV')
    parser.add_argument('--input-csv', 
                      type=str,
                      default='data/input/training_data.csv',
                      help='Path to input CSV file')
    parser.add_argument('--output-train',
                      type=str,
                      default='data/output/training_data.jsonl',
                      help='Path to output training data JSONL')
    parser.add_argument('--output-val',
                      type=str,
                      default='data/output/validation_data.jsonl', 
                      help='Path to output validation data JSONL')
    parser.add_argument('--train-ratio',
                      type=float,
                      default=0.8,
                      help='Ratio of data to use for training (default: 0.8)')
    parser.add_argument('--empty-examples',
                      type=int,
                      default=2000,
                      help='Number of empty examples to generate (default: 2000)')
    return parser.parse_args()

def read_tbl(input_path):
...
def generate_training_data(df, num_examples):
...
"""
    ...
    empty_examples = generate_empty_examples(num_examples)
...
def main():
    ...
    df = read_tbl(args.input_csv)
    
    # Generate training examples with specified number of empty examples
    examples = generate_training_data(df, num_examples=args.empty_examples)
    
    # Split into train and validation sets
    train_examples, val_examples = create_train_val_split(examples, args.train_ratio)
    
    # Save to specified paths
    save_training_data(train_examples, args.output_train)
    save_training_data(val_examples, args.output_val)
